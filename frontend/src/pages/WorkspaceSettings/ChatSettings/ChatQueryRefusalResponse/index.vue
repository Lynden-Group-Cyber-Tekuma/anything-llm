<template>
  <div>
    <div class="flex flex-col">
      <label for="name" class="block input-label">
        {{ t('chat.refusal.title') }}
      </label>
      <p class="text-white text-opacity-60 text-xs font-medium py-1.5">
        {{ t('chat.refusal.desc-start') }}
        <code class="border-none bg-theme-settings-input-bg p-0.5 rounded-sm">
          {{ t('chat.refusal.query') }}
        </code>
        {{ t('chat.refusal.desc-end') }}
      </p>
    </div>
    <textarea
      name="queryRefusalResponse"
      :rows="2"
      :defaultValue="chatQueryRefusalResponse(workspace)"
      class="border-none bg-theme-settings-input-bg placeholder:text-theme-settings-input-placeholder text-white text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5 mt-2"
      placeholder="The text returned in query mode when there is no relevant context found for a response."
      required
      wrap="soft"
      autocomplete="off"
      @change="$emit('update:hasChanges', true)"
    />
  </div>
</template>

<script setup>
import { useI18n } from 'vue-i18n';
import { chatQueryRefusalResponse } from '@/utils/chat';

defineProps({
  workspace: {
    type: Object,
    required: true,
  },
});

defineEmits(['update:hasChanges']);

const { t } = useI18n();
</script>
