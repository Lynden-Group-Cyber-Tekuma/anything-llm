...
const handleCreate = async (e) => {
  ...
  const { success, error: err } = await Invite.acceptInvite(code, data)
  if (success) {
...
<template>
  <div class="relative w-full max-w-2xl max-h-full">
    <div class="relative w-full max-w-2xl bg-theme-bg-secondary rounded-lg shadow border-2 border-theme-modal-border">
      <div class="flex items-start justify-between p-4 border-b rounded-t border-theme-modal-border">
        <h3 class="text-xl font-semibold text-theme-text-primary">
      ...
      <form @submit="handleCreate">
        <div class="p-6 space-y-6 flex h-full w-full">
          <div class="w-full flex flex-col gap-y-4">
            ...
            <p class="text-theme-text-secondary text-xs md:text-sm">
              After creating your account you will be able to login with these
              credentials and start using workspaces.
            </p>
          </div>
        </div>
        <div class="flex w-full justify-between items-center p-6 space-x-2 border-t rounded-b border-theme-modal-border">
          <button
            type="submit"
            class="w-full transition-all duration-300 border border-theme-text-primary px-4 py-2 rounded-lg text-theme-text-primary text-sm items-center flex gap-x-2 hover:bg-theme-text-primary hover:text-theme-bg-primary focus:ring-gray-800 text-center justify-center"
          >
            Accept Invitation
          </button>
        </div>
