...
const messageLimit = ref({
  enabled: props.user.dailyMessageLimit !== null,
  limit: props.user.dailyMessageLimit || 10
})
...
const handleUpdate = async (e) => {
  ...
  if (messageLimit.value.enabled) {
    data.dailyMessageLimit = messageLimit.value.limit
  } else {
    data.dailyMessageLimit = null
  }

  const { success, error: err } = await Admin.updateUser(props.user.id, data)
  if (success) {
    // Update local storage if we're editing our own user
    if (props.currentUser && props.currentUser.id === props.user.id) {
      props.currentUser.username = data.username
      props.currentUser.bio = data.bio
      props.currentUser.role = data.role
      localStorage.setItem(AUTH_USER, JSON.stringify(props.currentUser))
    }
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option v-if="currentUser?.role === 'admin'" value="admin">Administrator</option>
              </select>
