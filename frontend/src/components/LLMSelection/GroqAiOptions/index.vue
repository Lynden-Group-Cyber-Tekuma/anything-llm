...
const inputValue = ref(props.settings?.GroqApiKey)
const apiKey = ref(props.settings?.GroqApiKey)
...
const findCustomModels = async () => {
  ...
  try {
    loading.value = true
    const { models } = await System.customModels('groq', apiKey.value)
  ...
  <div class="flex gap-[36px] mt-1.5">
    <div class="flex flex-col w-60">
      ...
      <input
        ...
        :value="settings?.GroqApiKey ? '*'.repeat(20) : ''"
        required
        autocomplete="off"
        spellcheck="false"
    ...
    <div v-if="!settings?.credentialsOnly" class="flex flex-col w-60">
      ...
      <p class="text-xs leading-[18px] font-base text-white text-opacity-60 mt-2">
        {{ loading ? 'Enter a valid API key to view all available models for your account.' : 'Select the GroqAI model you want to use for your conversations.' }}
      </p>
    </div>
  </div>
</template>
